...
void	DOOM::Camera::render(DOOM::Doom const & doom, sf::Image & target, sf::Rect<int16_t> rect, int16_t palette)
{
  ...
  renderNode(doom, rect, (int16_t)(doom.level.nodes.size() - 1));
  ...
  // Draw things
  renderThings(doom, rect);

  // Draw pixel in image using palette
  for (int col = 0; col < (int)rect.width; col++)
    for (int row = 0; row < (int)rect.height; row++)
      target.setPixel(rect.left + col, rect.top + row, doom.resources.palettes[palette][doom.resources.colormaps[_buffer[col * rect.height + row].colormap][_buffer[col * rect.height + row].color]]);
}

bool	DOOM::Camera::renderNode(DOOM::Doom const & doom, sf::Rect<int16_t> rect, int16_t index)
{
  // Draw subsector if node ID has subsector mask
  if (index & 0b1000000000000000)
    return renderSubsector(doom, rect, index & 0b0111111111111111);
  ...
  if (Math::Vector<2>::determinant(node.direction, position.convert<2>() - node.origin) > 0.f)
    return renderNode(doom, rect, node.leftchild) == true ||
    ((Math::Vector<2>::determinant(node.origin - position.convert<2>(), node.direction) / Math::Vector<2>::determinant(_screen_start - position.convert<2>(), node.direction) >= 0.f || Math::Vector<2>::determinant(node.origin - position.convert<2>(), node.direction) / Math::Vector<2>::determinant(_screen_end - position.convert<2>(), node.direction) >= 0.f) && renderNode(doom, rect, node.rightchild) == true);
  else
    return renderNode(doom, rect, node.rightchild) == true ||
    ((Math::Vector<2>::determinant(node.origin - position.convert<2>(), node.direction) / Math::Vector<2>::determinant(_screen_start - position.convert<2>(), node.direction) >= 0.f || Math::Vector<2>::determinant(node.origin - position.convert<2>(), node.direction) / Math::Vector<2>::determinant(_screen_end - position.convert<2>(), node.direction) >= 0.f) && renderNode(doom, rect, node.leftchild) == true);
}

bool	DOOM::Camera::renderSubsector(DOOM::Doom const & doom, sf::Rect<int16_t> rect, int16_t index)
{
  ...
  for (int16_t i = 0; i < subsector.count; i++)
    if (renderSeg(doom, rect, subsector.index + i) == true)
      return true;

  return false;
}

bool	DOOM::Camera::renderSeg(DOOM::Doom const & doom, sf::Rect<int16_t> rect, int16_t index)
{
  ...
  for (int column = column_start; column < column_end; column++)
  {
    ...
    int16_t	shaded = renderLight(doom, rect, light, distance);
	...
	renderSky(doom, rect, column, 0, (int)std::lroundf(std::min(upper_front, upper_back)), sector_front.ceiling_current, index);
	_vertical[column].first = std::max(_vertical[column].first, (int)std::lroundf(std::min(upper_front, upper_back)));
      ...
      renderFlat(doom, rect, sector_front.ceiling_flat, column, 0, (int)std::lroundf(upper_front), sector_front.ceiling_current, light, index);
	...
	renderSky(doom, rect, column, (int)std::lroundf(std::max(lower_front, lower_back)), rect.height, sector_front.floor_current, index);
	_vertical[column].second = std::min(_vertical[column].second, (int)std::lroundf(std::max(lower_front, lower_back)));
      ...
      renderFlat(doom, rect, sector_front.floor_flat, column, (int)std::lroundf(lower_front), rect.height, sector_front.floor_current, light, index);
    ...
    if (upper_front < upper_back && texture_upper.height != 0 && (sector_front.ceiling_name != DOOM::str_to_key("F_SKY1") || sector_back.ceiling_name != DOOM::str_to_key("F_SKY1")))
      renderTexture(doom, rect, texture_upper, column, upper_front, upper_back, std::abs(sector_front.ceiling_current - sector_back.ceiling_current), texture_offset_x, upper_offset_y, shaded, index);
    if (lower_front > lower_back && texture_lower.height != 0 && (sector_front.floor_name != DOOM::str_to_key("F_SKY1") || sector_back.floor_name != DOOM::str_to_key("F_SKY1")))
      renderTexture(doom, rect, texture_lower, column, lower_back, lower_front, std::abs(sector_front.floor_current - sector_back.floor_current), texture_offset_x, lower_offset_y, shaded, index);
    if (upper_front < lower_front && texture_middle.height != 0)
      renderTexture(doom, rect, texture_middle, column, upper_front, lower_front, std::abs(sector_front.ceiling_current - sector_front.floor_current), texture_offset_x, middle_offset_y, shaded, index);
...
int16_t	DOOM::Camera::renderLight(const DOOM::Doom & doom, sf::Rect<int16_t> rect, int16_t light, float distance)
{
...
void	DOOM::Camera::renderTexture(DOOM::Doom const & doom, sf::Rect<int16_t> rect, DOOM::Doom::Resources::Texture const & texture, int column, float top, float bottom, float height, int offset_x, float offset_y, int16_t light, int16_t seg)
{
  ...
  for (int row = std::max(_vertical[column].first, (int)std::lroundf(top)); row < std::min((int)std::lroundf(bottom) + 0, _vertical[column].second); row++)
	...
	_buffer[column * rect.height + row] = { seg, std::numeric_limits<float>::quiet_NaN(), 31 - light / 8, span.pixels[pixel_y - span.offset] };
...
void	DOOM::Camera::renderFlat(DOOM::Doom const & doom, sf::Rect<int16_t> rect, const DOOM::AbstractFlat & flat, int column, int start, int end, float altitude, int16_t light, int16_t seg)
{
  ...
  for (int row = std::max(start, _vertical[column].first); row < std::min(end, _vertical[column].second); row++)
    if (_buffer[column * rect.height + row].color == -1)
    {
      ...
      // Compute light level according to distance TODO: improve this process?
      int16_t	shaded = renderLight(doom, rect, light, (position.convert<2>() - coord).length() / distance_distortion);
...
void	DOOM::Camera::renderSky(DOOM::Doom const & doom, sf::Rect<int16_t> rect, int column, int start, int end, float altitude, int16_t seg)
{
...
void	DOOM::Camera::renderThings(DOOM::Doom const & doom, sf::Rect<int16_t> rect)
{
  ...
  std::vector<DOOM::Camera::Pixel>	buffer_copy(rect.height);	// Copy the pixel buffer for Shadow things
  ...
  for (std::list<std::pair<const DOOM::AbstractThing&, float>>::const_iterator iterator = things.begin(); iterator != things.end(); iterator++)
  {
    ...
    int16_t	shaded = sprite.full_brightness == true ? 255 : renderLight(doom, rect, doom.level.sectors[thing_sector.first].light_current, iterator->second);
