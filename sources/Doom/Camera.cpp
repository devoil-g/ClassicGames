...
void	DOOM::Camera::render(DOOM::Doom const & doom, sf::Image & target, sf::Rect<int16_t> rect, DOOM::Camera::Special special, int16_t palette)
{
  ...
  renderNode(doom, rect, special, (int16_t)(doom.level.nodes.size() - 1));
...
bool	DOOM::Camera::renderNode(DOOM::Doom const & doom, sf::Rect<int16_t> rect, DOOM::Camera::Special special, int16_t index)
{
  // Draw subsector if node ID has subsector mask
  if (index & 0b1000000000000000)
    return renderSubsector(doom, rect, special, index & 0b0111111111111111);
  ...
  if (Math::Vector<2>::determinant(node.direction, position.convert<2>() - node.origin) > 0.f)
    return renderNode(doom, rect, special, node.leftchild) == true ||
    ((Math::Vector<2>::determinant(node.origin - position.convert<2>(), node.direction) / Math::Vector<2>::determinant(_screen_start - position.convert<2>(), node.direction) >= 0.f || Math::Vector<2>::determinant(node.origin - position.convert<2>(), node.direction) / Math::Vector<2>::determinant(_screen_end - position.convert<2>(), node.direction) >= 0.f) && renderNode(doom, rect, special, node.rightchild) == true);
  else
    return renderNode(doom, rect, special, node.rightchild) == true ||
    ((Math::Vector<2>::determinant(node.origin - position.convert<2>(), node.direction) / Math::Vector<2>::determinant(_screen_start - position.convert<2>(), node.direction) >= 0.f || Math::Vector<2>::determinant(node.origin - position.convert<2>(), node.direction) / Math::Vector<2>::determinant(_screen_end - position.convert<2>(), node.direction) >= 0.f) && renderNode(doom, rect, special, node.leftchild) == true);
}

bool	DOOM::Camera::renderSubsector(DOOM::Doom const & doom, sf::Rect<int16_t> rect, DOOM::Camera::Special special, int16_t index)
{
  ...
  for (int16_t i = 0; i < subsector.count; i++)
    if (renderSeg(doom, rect, special, subsector.index + i) == true)
      return true;

  return false;
}

bool	DOOM::Camera::renderSeg(DOOM::Doom const & doom, sf::Rect<int16_t> rect, DOOM::Camera::Special special, int16_t index)
{
  ...
  for (int column = column_start; column < column_end; column++)
  {
    ...
    if (upper_front < upper_back && texture_upper.height != 0 && (sector_front.ceiling_name != DOOM::str_to_key("F_SKY1") || sector_back.ceiling_name != DOOM::str_to_key("F_SKY1")))
      renderTexture(doom, rect, special, texture_upper, column, upper_front, upper_back, std::abs(sector_front.ceiling_current - sector_back.ceiling_current), texture_offset_x, upper_offset_y, shaded, index);
    if (lower_front > lower_back && texture_lower.height != 0 && (sector_front.floor_name != DOOM::str_to_key("F_SKY1") || sector_back.floor_name != DOOM::str_to_key("F_SKY1")))
      renderTexture(doom, rect, special, texture_lower, column, lower_back, lower_front, std::abs(sector_front.floor_current - sector_back.floor_current), texture_offset_x, lower_offset_y, shaded, index);
    if (upper_front < lower_front && texture_middle.height != 0)
      renderTexture(doom, rect, special, texture_middle, column, upper_front, lower_front, std::abs(sector_front.ceiling_current - sector_front.floor_current), texture_offset_x, middle_offset_y, shaded, index);
...
void	DOOM::Camera::renderFlat(DOOM::Doom const & doom, sf::Rect<int16_t> rect, DOOM::Camera::Special special, const DOOM::AbstractFlat & flat, int column, int start, int end, float altitude, int16_t light, int16_t seg)
{
...
void	DOOM::Camera::renderSky(DOOM::Doom const & doom, sf::Rect<int16_t> rect, DOOM::Camera::Special special, int column, int start, int end, float altitude, int16_t seg)
{
...
void	DOOM::Camera::renderThings(DOOM::Doom const & doom, sf::Rect<int16_t> rect, DOOM::Camera::Special special)
{
