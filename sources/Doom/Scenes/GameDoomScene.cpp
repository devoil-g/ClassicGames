  ...
  _doom(doom)
{
  ...
  if (_doom.level.episode == std::pair<std::uint8_t, std::uint8_t>{ 0, 0 })
...
void  DOOM::GameDoomScene::draw()
{
  ...
  // Compute grid size
  while (true) {
    if (((grid.first - 1) * DOOM::Doom::RenderScale + grid.first * DOOM::Doom::RenderWidth * DOOM::Doom::RenderScale) / ((grid.second - 1) * DOOM::Doom::RenderScale + grid.second * DOOM::Doom::RenderHeight * DOOM::Doom::RenderScale * DOOM::Doom::RenderStretching) > (float)Game::Window::Instance().getSize().x() / (float)Game::Window::Instance().getSize().y()) {
  ...
  if (_doom.image.getSize().x != (grid.first - 1) * DOOM::Doom::RenderScale + grid.first * DOOM::Doom::RenderWidth * DOOM::Doom::RenderScale || _doom.image.getSize().y != (grid.second - 1) * DOOM::Doom::RenderScale + grid.second * DOOM::Doom::RenderHeight * DOOM::Doom::RenderScale)
    _doom.image.resize({ (grid.first - 1) * DOOM::Doom::RenderScale + grid.first * DOOM::Doom::RenderWidth * DOOM::Doom::RenderScale, (grid.second - 1) * DOOM::Doom::RenderScale + grid.second * DOOM::Doom::RenderHeight * DOOM::Doom::RenderScale });
    ...
    for (int x = 0; x < grid.first; x++)
      if (y * grid.first + x < _doom.level.players.size()) {
        tasks.push_back(std::async(std::launch::async, [this, grid, x, y] {
          _doom.level.players[y * grid.first + x].get().draw(_doom, _doom.image, Math::Box<2, std::int16_t>(
            { (std::int16_t)(x * DOOM::Doom::RenderScale + x * DOOM::Doom::RenderWidth * DOOM::Doom::RenderScale), (std::int16_t)(y * DOOM::Doom::RenderScale + y * DOOM::Doom::RenderHeight * DOOM::Doom::RenderScale) },
            { (std::int16_t)(DOOM::Doom::RenderWidth * DOOM::Doom::RenderScale), (std::int16_t)(DOOM::Doom::RenderHeight * DOOM::Doom::RenderScale) }
          ), DOOM::Doom::RenderScale);
        }));
      }
...
{
  std::pair<std::uint8_t, std::uint8_t> previous = _doom.level.episode;
  std::pair<std::uint8_t, std::uint8_t> next = { 0, 0 };
  ...
  {
    const std::map<std::pair<std::uint8_t, std::uint8_t>, std::pair<std::uint8_t, std::uint8_t>>  shareware = {
  ...
  {
    const std::map<std::pair<std::uint8_t, std::uint8_t>, std::pair<std::uint8_t, std::uint8_t>>  commercial = {
      { { 1, 1 }, { 1, 2 } },   // ENTRYWAY
      { { 1, 2 }, { 1, 3 } },   // UNDERHALLS
      { { 1, 3 }, { 1, 4 } },   // THE GANTLET
      { { 1, 4 }, { 1, 5 } },   // THE FOCUS
    ...
    if (_doom.level.end == DOOM::Enum::End::EndSecret)
      next = (_doom.level.episode == std::pair<std::uint8_t, std::uint8_t>(1, 31)) ? std::pair<std::uint8_t, std::uint8_t>(1, 32) : std::pair<std::uint8_t, std::uint8_t>(1, 31);
  ...
  {
    const std::map<std::pair<std::uint8_t, std::uint8_t>, std::pair<std::uint8_t, std::uint8_t>>  registered = {
      // KNEE-DEEP IN THE DEAD
      { { 1, 1 }, { 1, 2 } }, // HANGAR
      { { 1, 2 }, { 1, 3 } }, // NUCLEAR PLANT
      { { 1, 3 }, { 1, 4 } }, // TOXIN REFINERY
  ...
  {
    const std::map<std::pair<std::uint8_t, std::uint8_t>, std::pair<std::uint8_t, std::uint8_t>> retail = {
      // KNEE-DEEP IN THE DEAD
      { { 1, 1 }, { 1, 2 } }, // HANGAR
      { { 1, 2 }, { 1, 3 } }, // NUCLEAR PLANT
      { { 1, 3 }, { 1, 4 } }, // TOXIN REFINERY
  ...
  std::pair<std::uint64_t, std::string> text = { 0, "" };
  ...
  case DOOM::Enum::Mode::ModeRetail:      // DOOM 1 retail, E4, M36
  {
    const std::array<std::pair<std::uint64_t, std::string>, 4> doom = {
      std::pair<std::uint64_t, std::string>(Game::Utilities::str_to_key<std::uint64_t>("FLOOR4_8"), std::string("Once you beat the big badasses and \nclean out the moon base you're supposed\nto win, aren't you? Aren't you? Where's\nyour fat reward and ticket home? What\nthe hell is this? It's not supposed to\nend this way!\n\nIt stinks like rotten meat, but looks\nlike the lost Deimos base.  Looks like\nyou're stuck on The Shores of Hell.\nThe only way out is through.\n\nTo continue the DOOM experience, play\nThe Shores of Hell and its amazing\nsequel, Inferno!\n")),
      std::pair<std::uint64_t, std::string>(Game::Utilities::str_to_key<std::uint64_t>("SFLR6_1"), std::string("You've done it! The hideous cyber-\ndemon lord that ruled the lost Deimos\nmoon base has been slain and you\nare triumphant! But ... where are\nyou? You clamber to the edge of the\nmoon and look down to see the awful\ntruth.\n\nDeimos floats above Hell itself!\nYou've never heard of anyone escaping\nfrom Hell, but you'll make the bastards\nsorry they ever heard of you! Quickly,\nyou rappel down to  the surface of\nHell.\n\nNow, it's on to the final chapter of\nDOOM! -- Inferno.")),
      std::pair<std::uint64_t, std::string>(Game::Utilities::str_to_key<std::uint64_t>("MFLR8_4"), std::string("The loathsome spiderdemon that\nmasterminded the invasion of the moon\nbases and caused so much death has had\nits ass kicked for all time.\n\nA hidden doorway opens and you enter.\nYou've proven too tough for Hell to\ncontain, and now Hell at last plays\nfair -- for you emerge from the door\nto see the green fields of Earth!\nHome at last.\n\nYou wonder what's been happening on\nEarth while you were battling evil\nunleashed. It's good that no Hell-\nspawn could have come through that\ndoor with you ...")),
      std::pair<std::uint64_t, std::string>(Game::Utilities::str_to_key<std::uint64_t>("MFLR8_3"), std::string("the spider mastermind must have sent forth\nits legions of hellspawn before your\nfinal confrontation with that terrible\nbeast from hell.  but you stepped forward\nand brought forth eternal damnation and\nsuffering upon the horde as a true hero\nwould in the face of something so evil.\n\nbesides, someone was gonna pay for what\nhappened to daisy, your pet rabbit.\n\nbut now, you see spread before you more\npotential pain and gibbitude as a nation\nof demons run amok among our cities.\n\nnext stop, hell on earth!"))
    };

    // Text at the end of the episode
    if (previous.first >= 1 && previous.first <= doom.size() && next == std::pair<std::uint8_t, uint8_t>(0, 0))
      text = doom[previous.first - 1];
  } break;

  case DOOM::Enum::Mode::ModeCommercial:  // DOOM 2 retail, E1 M34
  {
    const std::map<std::uint8_t, std::pair<std::uint64_t, std::string>> doom2 = {
      { 7, { Game::Utilities::str_to_key<std::uint64_t>("SLIME16"), std::string("YOU HAVE ENTERED DEEPLY INTO THE INFESTED\nSTARPORT. BUT SOMETHING IS WRONG. THE\nMONSTERS HAVE BROUGHT THEIR OWN REALITY\nWITH THEM, AND THE STARPORT'S TECHNOLOGY\nIS BEING SUBVERTED BY THEIR PRESENCE.\n\nAHEAD, YOU SEE AN OUTPOST OF HELL, A\nFORTIFIED ZONE. IF YOU CAN GET PAST IT,\nYOU CAN PENETRATE INTO THE HAUNTED HEART\nOF THE STARBASE AND FIND THE CONTROLLING\nSWITCH WHICH HOLDS EARTH'S POPULATION\nHOSTAGE.") } },
      { 12, { Game::Utilities::str_to_key<std::uint64_t>("RROCK14"), std::string("YOU HAVE WON! YOUR VICTORY HAS ENABLED\nHUMANKIND TO EVACUATE EARTH AND ESCAPE\nTHE NIGHTMARE.  NOW YOU ARE THE ONLY\nHUMAN LEFT ON THE FACE OF THE PLANET.\nCANNIBAL MUTATIONS, CARNIVOROUS ALIENS,\nAND EVIL SPIRITS ARE YOUR ONLY NEIGHBORS.\nYOU SIT BACK AND WAIT FOR DEATH, CONTENT\nTHAT YOU HAVE SAVED YOUR SPECIES.\n\nBUT THEN, EARTH CONTROL BEAMS DOWN A\nMESSAGE FROM SPACE: \"SENSORS HAVE LOCATED\nTHE SOURCE OF THE ALIEN INVASION. IF YOU\nGO THERE, YOU MAY BE ABLE TO BLOCK THEIR\nENTRY.  THE ALIEN BASE IS IN THE HEART OF\nYOUR OWN HOME CITY, NOT FAR FROM THE\nSTARPORT.\" SLOWLY AND PAINFULLY YOU GET\nUP AND RETURN TO THE FRAY.") } },
      { 21, { Game::Utilities::str_to_key<std::uint64_t>("RROCK07"), std::string("YOU ARE AT THE CORRUPT HEART OF THE CITY,\nSURROUNDED BY THE CORPSES OF YOUR ENEMIES.\nYOU SEE NO WAY TO DESTROY THE CREATURES'\nENTRYWAY ON THIS SIDE, SO YOU CLENCH YOUR\nTEETH AND PLUNGE THROUGH IT.\n\nTHERE MUST BE A WAY TO CLOSE IT ON THE\nOTHER SIDE. WHAT DO YOU CARE IF YOU'VE\nGOT TO GO THROUGH HELL TO GET TO IT?") } },
      { 0, { Game::Utilities::str_to_key<std::uint64_t>("RROCK17"), std::string("THE HORRENDOUS VISAGE OF THE BIGGEST\nDEMON YOU'VE EVER SEEN CRUMBLES BEFORE\nYOU, AFTER YOU PUMP YOUR ROCKETS INTO\nHIS EXPOSED BRAIN. THE MONSTER SHRIVELS\nUP AND DIES, ITS THRASHING LIMBS\nDEVASTATING UNTOLD MILES OF HELL'S\nSURFACE.\n\nYOU'VE DONE IT. THE INVASION IS OVER.\nEARTH IS SAVED. HELL IS A WRECK. YOU\nWONDER WHERE BAD FOLKS WILL GO WHEN THEY\nDIE, NOW. WIPING THE SWEAT FROM YOUR\nFOREHEAD YOU BEGIN THE LONG TREK BACK\nHOME. REBUILDING EARTH OUGHT TO BE A\nLOT MORE FUN THAN RUINING IT WAS.\n") } },
      { 31, { Game::Utilities::str_to_key<std::uint64_t>("RROCK13"), std::string("CONGRATULATIONS, YOU'VE FOUND THE SECRET\nLEVEL! LOOKS LIKE IT'S BEEN BUILT BY\nHUMANS, RATHER THAN DEMONS. YOU WONDER\nWHO THE INMATES OF THIS CORNER OF HELL\nWILL BE.") } },
      { 32, { Game::Utilities::str_to_key<std::uint64_t>("RROCK19"), std::string("CONGRATULATIONS, YOU'VE FOUND THE\nSUPER SECRET LEVEL!  YOU'D BETTER\nBLAZE THROUGH THIS ONE!\n") } }
    };
  ...
  // Load next level
  if (next != std::pair<std::uint8_t, std::uint8_t>(0, 0)) {
    doom.setLevel(next);
