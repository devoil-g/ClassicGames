...
void  QUIZ::FastestQuizScene::setCorrect()
{
  ...
  for (int index = 1; index <= 4; index++) {
    ...
    for (int pos = 0; pos < 4; pos++)
      if (answer.getText() == _quiz.fastests.at(_question).answers.at(pos) && positions[pos] != 0.f) {
  ...
  // Display question infos:
  std::wcout << std::endl
    << "Info:" << std::endl
    << _quiz.fastests.at(_question).info << std::endl << std::endl;
}
...
void  QUIZ::FastestQuizScene::setWinner()
{
  ...
  for (unsigned int index = 0; index < _answers.size(); index++) {
    ...
    if (_answers[index].first < best && entity.getTargetColor()(3) == 1.f) {
